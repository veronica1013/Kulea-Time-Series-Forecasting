...
print(col_names)
sel_data <- data[, c("Month", "High-Quality Prod. est"
)]
print(head(sel_data))
print(class(sel_data))
end_date <- as.Date("2026-04-01 00:00:00")
sel_data_2 <- sel_data[sel_data$Month <= end_date, ]
print(tail(sel_data_2))
ts_data <- ts(sel_data_2$`High-Quality Prod. est`,
...
arima_model <- Arima(ts_data,
                     order = c(1, 1, 4), #
                     seasonal = list(order = c(0, 2, 1), period = 36))
...
file_path <- "forecast_south_africa_high_q_prod_2026_2028.csv"
