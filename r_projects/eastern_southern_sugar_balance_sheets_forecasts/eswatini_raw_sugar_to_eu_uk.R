...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Eswatini_Data_2_.xlsx",
                   sheet = "Eswatini", skip = 1)
head(data)
num_col <- ncol(data)
col_names <- colnames(data)
print(col_names)
sel_data <- data[, c("Month", "Raw sugar to EU/UK"
)]
print(head(sel_data))
...
ts_data <- ts(sel_data_2$`Raw sugar to EU/UK`,
...
# file_path <- "forecast_eswatini_raw_sugar_to_eu_uk_2026_2028.csv"
