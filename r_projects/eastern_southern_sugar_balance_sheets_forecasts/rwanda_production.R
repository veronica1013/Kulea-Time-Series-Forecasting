...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Rwanda.xlsx",
                   sheet = "Rwanda_Copy", skip = 1)
head(data)
num_col <- ncol(data)
...
arima_model <- Arima(ts_data,
                     order = c(0, 0, 1), # 0,0,1,0,1,1,12
                     seasonal = list(order = c(0, 1, 1), period = 120))
...
forecast_rwanda_production <- forecast(arima_model, level = c(95), h = 2 * 12)
plot(forecast_rwanda_production)
print(forecast_rwanda_production)
...
file_path <- "forecast_rwanda_production_2026_2028.csv"

# Save the forecast to a CSV file
write.csv(forecast_rwanda_production, file = file_path, row.names = TRUE)