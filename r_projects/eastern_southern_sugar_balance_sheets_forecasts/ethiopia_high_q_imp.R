...
print(col_names)
sel_data <- data[, c("Month", "High-Quality(White) Imp"
)]
print(head(sel_data))
...
ts_data <- ts(sel_data_2$`High-Quality(White) Imp`,
...
arima_model <- Arima(ts_data,
                     order = c(1, 0, 1),
                     seasonal = list(order = c(0, 2, 1), period = 24))
...
forecast_ethiopia_high_q_imp <- forecast(arima_model, level = c(95), h = 2 * 12)
plot(forecast_ethiopia_high_q_imp)
print(forecast_ethiopia_high_q_imp)
...
file_path <- "forecast_ethiopia_high_q_imports_2026_2027.csv"

# Save the forecast to a CSV file
write.csv(forecast_ethiopia_high_q_imp, file = file_path, row.names = TRUE)