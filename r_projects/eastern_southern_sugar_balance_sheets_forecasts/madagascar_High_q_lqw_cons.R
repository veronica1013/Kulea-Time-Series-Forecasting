...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Madagascar.xlsx",
                   sheet = "Madagascar_Copy", skip = 1)
head(data)
num_col <- ncol(data)
col_names <- colnames(data)
print(col_names)
sel_data <- data[, c("Month", "High-Quality/LQW Cons"
)]
print(head(sel_data))
print(class(sel_data))
end_date <- as.Date("2026-03-01 00:00:00")
sel_data_2 <- sel_data[sel_data$Month <= end_date, ]
print(tail(sel_data_2))
ts_data <- ts(sel_data_2$`High-Quality/LQW Cons`,
...
forecast_eastern_congo_consumption <- forecast(arima_model, level = c(95), h = 2 * 12)
plot(forecast_eastern_congo_consumption)
print(forecast_eastern_congo_consumption)
...
file_path <- "forecast_madagascar_high_q_lqw_consumption_2026_2028.csv"

# Save the forecast to a CSV file
write.csv(forecast_eastern_congo_consumption, file = file_path, row.names = TRUE)