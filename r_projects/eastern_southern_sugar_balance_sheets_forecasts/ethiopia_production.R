...
arima_model <- Arima(ts_data,
                     order = c(1, 1, 0),
                     seasonal = list(order = c(2, 1, 2), period = 60))
...
forecast_ethiopia_production <- forecast(arima_model, level = c(95), h = 2 * 12)
# plot(forecast_ethiopia_production)
print(forecast_ethiopia_production)
...
# file_path <- "forecast_ethiopia_production_2026_2027.csv"

# # Save the forecast to a CSV file
# write.csv(forecast_ethiopia_production, file = file_path, row.names = TRUE)