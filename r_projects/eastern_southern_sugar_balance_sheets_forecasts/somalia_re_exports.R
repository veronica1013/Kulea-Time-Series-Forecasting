...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Somalia.xlsx",
                   sheet = "Somalia", skip = 1)
head(data)
num_col <- ncol(data)
col_names <- colnames(data)
print(col_names)
sel_data <- data[, c("Month", "Re-Exports"
)]
print(head(sel_data))
print(class(sel_data))
end_date <- as.Date("2023-12-01 00:00:00")
sel_data_2 <- sel_data[sel_data$Month <= end_date, ]
print(tail(sel_data_2))
ts_data <- ts(sel_data_2$`Re-Exports`,
...
arima_model <- Arima(ts_data,
                     order = c(0, 1, 2), #
                     seasonal = list(order = c(0, 1, 2), period = 60))
...
file_path <- "forecast_somalia_re_exports_2026_2028.csv"
