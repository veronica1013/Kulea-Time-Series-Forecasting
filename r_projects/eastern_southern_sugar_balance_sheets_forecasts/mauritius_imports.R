...
arima_model <- Arima(ts_data,
                     order = c(2, 1, 2),
                     seasonal = list(order = c(0, 2, 3), period = 24))
...
forecast_mauritius_imports <- forecast(arima_model, level = c(95), h = 2 * 12)
plot(forecast_mauritius_imports)
print(forecast_mauritius_imports)
...
file_path <- "forecast_mauritius_imports_1_2026_2028.csv"

# Save the forecast to a CSV file
write.csv(forecast_mauritius_imports, file = file_path, row.names = TRUE)