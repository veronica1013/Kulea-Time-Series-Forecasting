...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\South_Africa.xlsx",
                   sheet = "SouthAfrica_Copy", skip = 1)
head(data)
num_col <- ncol(data)
col_names <- colnames(data)
print(col_names)
sel_data <- data[, c("Month", "High-Quality Cons. Est"
)]
print(head(sel_data))
...
ts_data <- ts(sel_data_2$`High-Quality Cons. Est`,
...
arima_model <- Arima(ts_data,
                     order = c(0, 1, 0), #
                     seasonal = list(order = c(0, 0, 2), period = 24)) #2, 1,1
...
forecast_south_africa_production <- forecast(arima_model, level = c(95), h = 2 * 12)
plot(forecast_south_africa_production)
print(forecast_south_africa_production)
...
file_path <- "forecast_south_africa_high_q_cons_2026_2028.csv"

# Save the forecast to a CSV file
write.csv(forecast_south_africa_production, file = file_path, row.names = TRUE)