...
library(tseries)
library(dplyr)
data_2 <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Botswana_Data_2.xlsx",
...
print(col_names)
sel_data <- data[, c("Month", "Consumption", "High-Quality white Imp"
)]
print(head(sel_data))
...
import_ts_data <- ts(sel_data_2$`High-Quality white Imp`,
                     start = c(year(sel_data_2$Month[1]),
                               month(sel_data_2$Month[1])),
                     frequency = 12)
...
arima_model <- Arima(import_ts_data,
                     order = c(0, 0, 0),
                     xreg = cons_ts_data,
                     seasonal = list(order = c(0, 1, 0), period = 36))
...
forecasted_imports_data <- forecast(arima_model, xreg = cons_data, h = 4)
...
file_path <- "forecast_botswana_high_q_imp_2023_2026.csv"
