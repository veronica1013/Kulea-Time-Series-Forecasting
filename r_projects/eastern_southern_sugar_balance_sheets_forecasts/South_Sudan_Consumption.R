...
print(col_names)
sel_data <- data[, c("Month", "Bagged VHP/LQW Cons"
)]
print(head(sel_data))
...
ts_data <- ts(sel_data_2$`Bagged VHP/LQW Cons`,
...
arima_model <- Arima(ts_data,
                     order = c(0, 1, 0), #
                     seasonal = list(order = c(0, 1, 0), period = 24)) #2, 1,1
...
file_path <- "forecast_south_sudan_vhp_consumption_2026_2028.csv"
