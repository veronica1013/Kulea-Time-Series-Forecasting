...
library(tseries)
library(dplyr)
data_2 <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Eswatini_Data_2.xlsx",
                    sheet = "Eswatini", skip = 1)
# Filter data for consumption from 2015 to 2026
export_data <- data_2 %>%
filter(year(Month) >= 2026 & year(Month) <= 2028) %>% select(Month, Exports)

print(tail(export_data))

exports_data <- ts(export_data$Exports,
...
print(col_names)
sel_data <- data[, c("Month", "Exports", "Raw Exp"
)]
print(head(sel_data))
...
export_ts_data <- ts(sel_data_2$Exports,
                     start = c(year(sel_data_2$Month[1]),
                               month(sel_data_2$Month[1])),
                     frequency = 12)

raw_exports_ts_data <- ts(sel_data_2$`Raw Exp`,
                          start = c(year(sel_data_2$Month[1]),
                                    month(sel_data_2$Month[1])),
                          frequency = 12)
...
arima_model <- Arima(raw_exports_ts_data,
                     order = c(0, 3, 4),
                     xreg = export_ts_data,
                     seasonal = list(order = c(0, 1, 1), period = 60))
...
# file_path <- "forecast_eswatini_raw_exports_2026_2028.csv"
