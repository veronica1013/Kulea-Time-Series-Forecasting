...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Sudan_Data_2.xlsx",
                   sheet = "Sudan_Copy", skip = 1)
head(data)
num_col <- ncol(data)
col_names <- colnames(data)
print(col_names)
sel_data <- data[, c("Month", "High-Quality Prod"
)]
print(head(sel_data))
...
ts_data <- ts(sel_data_2$`High-Quality Prod`,
...
cycle(ts_data)
boxplot(ts_data ~ cycle(ts_data))
missing_values <- is.na(ts_data)
print(sum(missing_values))
arima_model <- Arima(ts_data,
                     order = c(1, 0, 0), #
                     seasonal = list(order = c(0, 1, 0), period = 60))
...
r_squared <- summary(arima_model)$r.squared

# Print the R-squared with a meaningful message
cat("R-squared (R2) Score:", format(r_squared, nsmall = 4), "\n")
acf(arima_model$residuals, main = "ACF Residual")
# pacf(arima_model$residuals, main = "PACF Residual")
forecast_sudan_production <- forecast(arima_model, level = c(95), h = 2 * 12)
plot(forecast_sudan_production)
print(forecast_sudan_production)
...
# file_path <- "forecast_sudan_production_2026_2028.csv"

# # Save the forecast to a CSV file
# write.csv(forecast_sudan_production, file = file_path, row.names = TRUE)