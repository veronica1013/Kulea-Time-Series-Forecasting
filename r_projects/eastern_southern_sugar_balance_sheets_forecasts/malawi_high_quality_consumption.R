...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Malawi.xlsx",
                   sheet = "Malawi_Copy", skip = 1)
head(data)
num_col <- ncol(data)
col_names <- colnames(data)
print(col_names)
sel_data <- data[, c("Month", "High-Quality Cons"
)]
print(head(sel_data))
...
ts_data <- ts(sel_data_2$`High-Quality Cons`,
...
file_path <- "forecast_malawi_high_quality_consumption_2026_2028.csv"
