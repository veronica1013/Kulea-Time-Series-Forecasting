...
print(col_names)
sel_data <- data[, c("Month", "Low-Quality(Crystal) Cons"
)]
print(head(sel_data))
...
ts_data <- ts(sel_data_2$`Low-Quality(Crystal) Cons`,
...
arima_model <- Arima(ts_data,
                     order = c(0, 1, 0),
                     seasonal = list(order = c(0, 1, 1), period = 72))
...
forecast_djibouti_low_cons <- forecast(arima_model, level = c(95), h = 4 * 12)
# plot(forecast_djibouti_low_cons)
print(forecast_djibouti_low_cons)
...
# file_path <- "forecast_djibouti_low_cons_2026_2027.csv"

# # Save the forecast to a CSV file
# write.csv(forecast_djibouti_low_cons, file = file_path, row.names = TRUE)