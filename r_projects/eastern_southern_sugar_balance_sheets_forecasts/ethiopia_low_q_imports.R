...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Ethiopia.xlsx",
                   sheet = "Ethiopia_Copy", skip = 1)
head(data)
num_col <- ncol(data)
col_names <- colnames(data)
print(col_names)
sel_data <- data[, c("Month", "Low-Quality(Brown/Crystal) Imp"
)]
print(head(sel_data))
print(class(sel_data))
end_date <- as.Date("2026-09-01 00:00:00")
sel_data_2 <- sel_data[sel_data$Month <= end_date, ]
print(tail(sel_data_2))
ts_data <- ts(sel_data_2$`Low-Quality(Brown/Crystal) Imp`,
...
arima_model <- Arima(ts_data,
                     order = c(1, 1, 1),
                     seasonal = list(order = c(0, 2, 2), period = 36))
...
forecast_ethiopia_low_q_imp <- forecast(arima_model, level = c(95), h = 2 * 12)
plot(forecast_ethiopia_low_q_imp)
print(forecast_ethiopia_low_q_imp)
...
# file_path <- "forecast_ethiopia_low_q_imports_2026_2027.csv"

# # Save the forecast to a CSV file
# write.csv(forecast_ethiopia_low_q_imp, file = file_path, row.names = TRUE)