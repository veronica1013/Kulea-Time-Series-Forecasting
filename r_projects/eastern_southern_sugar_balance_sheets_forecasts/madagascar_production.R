...
print(col_names)
sel_data <- data[, c("Month", "Bagged VHP Prod"
)]
print(head(sel_data))
...
ts_data <- ts(sel_data_2$`Bagged VHP Prod`,
...
arima_model <- Arima(ts_data,
                     order = c(0, 0, 1),
                     seasonal = list(order = c(0, 1, 1), period = 36))
...
# file_path <- "forecast_madagascar_bagged_3_vhp_production_2026_2028.csv"
