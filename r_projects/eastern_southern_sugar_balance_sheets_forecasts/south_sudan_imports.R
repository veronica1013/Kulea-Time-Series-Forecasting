...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\South_Sudan.xlsx",
                   sheet = "South_Sudan_Copy", skip = 1)
head(data)
num_col <- ncol(data)
...
end_date <- as.Date("2022-12-01 00:00:00")
...
arima_model <- Arima(ts_data,
                     order = c(1, 0, 0), #
                     seasonal = list(order = c(0, 1, 0), period = 48)) #2, 1,1
...
forecast_south_africa_production <- forecast(arima_model, level = c(95), h = 5 * 12)
...
file_path <- "forecast_south_sudan_imports_updated_2026_2028.csv"
