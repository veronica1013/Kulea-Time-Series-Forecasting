...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Djibouti.xlsx",
                   sheet = "Djibouti_Copy", skip = 1)
head(data)
num_col <- ncol(data)
...
# arima_model <- Arima(ts_data,
#                      order = c(1, 1, 1),
#                      seasonal = list(order = c(0, 1, 1), period = 156))
...
forecast_djibouti_re_exports <- forecast(arima_model, level = c(95), h = 4 * 12)
plot(forecast_djibouti_re_exports)
print(forecast_djibouti_re_exports)
...
file_path <- "forecast_djibouti_re_exports_2026_2027.csv"

# Save the forecast to a CSV file
write.csv(forecast_djibouti_re_exports, file = file_path, row.names = TRUE)