...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Burundi.xlsx",
                   sheet = "Burundi_Copy", skip = 1)
head(data)
num_col <- ncol(data)
...
arima_model <- Arima(ts_data,
                     order = c(0, 1, 2),
                     seasonal = list(order = c(2, 2, 0), period = 30))
...
forecast_burundi_imports <- forecast(arima_model, level = c(95), h = 2 * 12)
# plot(forecast_burundi_imports)
print(forecast_burundi_imports)
...
# file_path <- "forecast_burundi_imports_2026_2027.csv"

# # Save the forecast to a CSV file
# write.csv(forecast_burundi_imports, file = file_path, row.names = TRUE)