...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Namibia.xlsx",
                   sheet = "Namibia_Copy", skip = 1)
head(data)
num_col <- ncol(data)
col_names <- colnames(data)
print(col_names)
sel_data <- data[, c("Month", "High-Quality White Cons"
)]
print(head(sel_data))
...
ts_data <- ts(sel_data_2$`High-Quality White Cons`,
...
arima_model <- Arima(ts_data,
                     order = c(0, 1, 0),
                     seasonal = list(order = c(0, 0, 2), period = 24))
...
forecast_namibia_consumption <- forecast(arima_model, level = c(95), h = 2 * 12)
plot(forecast_namibia_consumption)
print(forecast_namibia_consumption)
...
# file_path <- "forecast_namibia_high_q_consumption_2026_2028.csv"

# # Save the forecast to a CSV file
# write.csv(forecast_namibia_consumption, file = file_path, row.names = TRUE)