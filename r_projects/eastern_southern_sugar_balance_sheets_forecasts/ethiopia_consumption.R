...
arima_model <- Arima(ts_data,
                     order = c(0, 1, 4),
                     seasonal = list(order = c(0, 0, 2), period = 96))
...
forecast_ethiopia_consumption <- forecast(arima_model, level = c(95), h = 2 * 12)
plot(forecast_ethiopia_consumption)
print(forecast_ethiopia_consumption)
...
file_path <- "forecast_ethiopia_consumption_2026_2027.csv"

# Save the forecast to a CSV file
write.csv(forecast_ethiopia_consumption, file = file_path, row.names = TRUE)