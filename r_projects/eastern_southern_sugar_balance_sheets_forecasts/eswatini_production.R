...
arima_model <- Arima(ts_data,
                     order = c(1, 0, 0),
                     seasonal = list(order = c(0, 2, 2), period = 24))
...
forecast_eswatini_production <- forecast(arima_model, level = c(95), h = 2 * 12)
plot(forecast_eswatini_production)
print(forecast_eswatini_production)
...
# file_path <- "forecast_eswatini_production_2026_2028.csv"

# # Save the forecast to a CSV file
# write.csv(forecast_eswatini_production, file = file_path, row.names = TRUE)