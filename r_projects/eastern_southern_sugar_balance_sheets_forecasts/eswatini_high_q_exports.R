...
library(readxl)
library(tseries)
data <- read_excel("D:\\kulea_projects\\r_project\\east_south_sugar\\Eswatini.xlsx",
                   sheet = "Eswatini_Copy", skip = 1)
head(data)
num_col <- ncol(data)
col_names <- colnames(data)
print(col_names)
sel_data <- data[, c("Month", "High-quality white exp"
)]
print(head(sel_data))
...
ts_data <- ts(sel_data_2$`High-quality white exp`,
...
arima_model <- Arima(ts_data,
                     order = c(1, 1, 1),
                     seasonal = list(order = c(0, 0, 0), period = 60))
...
file_path <- "forecast_eswatini_high_q_exports_2026_2028.csv"
